...
public class PublicAdapter extends FirebaseRecyclerPagingAdapter<PostData, PublicAdapter.Publicview> {
    Context context;


    public PublicAdapter(@NonNull com.firebase.ui.database.paging.DatabasePagingOptions<PostData> options, Context context) {
        super(options);
    ...
    protected void onBindViewHolder(@NonNull Publicview viewHolder, int position, @NonNull PostData model) {
        ...
        if (model.getImageLink().equals("")) {
            ...
            if (viewHolder.download.getVisibility() == View.VISIBLE) viewHolder.download.setVisibility(View.GONE);
        } else {
